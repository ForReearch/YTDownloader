...
class MediaDownloaderGUI:
    def __init__(self, root):
        ...
        tk.Label(self.header_frame, text="Frog Downloader (YouTube, Instagram, Facebook) កំណែរទម្រង់ V 0.00", font=("Helvetica", 16, "bold"), fg='white', bg=self.colors['header']).pack()
        ...
        #góógletrans: Multiple videos/posts/reels
    ...
    def download_channel_thread(self, url):
        ...
        with yt_dlp.YoutubeDL(opts) as ydl:
            try:
                ydl.download([url])
                self.log_status(f"✅ ទាញយកឆានែល/បញ្ជីចាក់/ប្រវត្តិរូប/ទំព័របានសម្រេច: {url} ➜ រក្សាទុកនៅ '{DOWNLOAD_FOLDER}'")
            except Exception as e:
                self.log_status(f"❌ បរាជ័យ: {url}: {e}")
            finally:
                self.root.after(0, self.progress.stop)

...
DEFAULT_DOWNLOAD_FOLDER = "Video Downloaded"
...
def load_khmer_font():
    ...
    # Note: Loading TTF directly is not supported in standard Tkinter.
    # Ensure 'Khmer OS Siemreap' is installed on the system.
...
class MediaDownloaderGUI:
    def __init__(self, root):
        ...
        # Load or set default download folder
        ...
        self.log_status(f"ទីតាំងរក្សាទុកបច្ចុប្បន្ន: {self.download_folder}")
        ...
        with yt_dlp.YoutubeDL(self.get_ydl_opts()) as ydl:
            for url in urls:
                try:
                    ...
                    self.log_status(f"❌ បរាជ័យ: {url}: {e}")
            self.log_status(f"✅ ទាញយកទាំងអស់បានបញ្ចប់ ➜ រក្សាទុកនៅ '{self.download_folder}'")
        self.root.after(0, self.progress.stop)
    ...
    def download_channel_thread(self, url):
        ...
        opts['noplaylist'] = False  # Allow playlists/channels/profiles/pages

...
class MediaDownloaderGUI:
        ...
        for frame in self.main_frame.winfo_children():
            ...
            for widget in frame.winfo_children():
                if isinstance(widget, tk.Label):
                    widget.configure(bg=self.colors['card'], fg=self.colors['text'] if widget.cget("font") == str(self.label_font) else self.colors['text2'])
    ...
    def download_single(self):
        ...
        if not self.prompt_for_download_folder():
            self.log_status("❌ កំហុស: សូមជ្រើសរើសទីតាំងរក្សាទុក។")
            return
        self.progress.start()

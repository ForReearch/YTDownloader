...
class MediaDownloaderGUI:
    def __init__(self, root):
        ...
        style.configure("Modern.TButton", 
                        ...
                        padding=(15, 8),  # Increased padding for curved effect
        ...
        style.map("Modern.TButton",
                 background=[('active', self.colors['accent_active']),
                            ('!disabled', '!active', self.colors['accent']),
                            ('disabled', '#A0A0A0')],
                 foreground=[('active', 'white'),
                            ('Says: ('!disabled', '!active', 'white')'),
                            ('disabled', '#606770')])
        ...
        ttk.Button(single_frame, text="ទាញយកមួយ", style="Modern.TButton", command=self.download_single).grid(row=2, column=1, padx=5, pady=5)
        ...
        ttk.Button(multi_frame, text="ទាញយកច្រើន", style="Modern.TButton", command=self.download_multiple).grid(row=2, column=1, padx=5, pady=5)
        ...
        ttk.Button(channel_frame, text="ទាញយកឆានែល/បញ្ជីចាក់/ប្រវត្តិរូប/ទំព័រ", style="Modern.TButton", command=self.download_channel).grid(row=2, column=1, padx=5, pady=5)
        ...
        ttk.Button(footer_frame, text="ចាកចេញ", style="Modern.TButton", command=self.root.quit).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(footer_frame, text="រក្សាទុក", style="Modern.TButton", command=self.set_download_folder).pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(footer_frame, text="បើក", style="Modern.TButton", command=self.open_download_folder).pack(side=tk.LEFT, padx=5, pady=5)
        self.theme_button = ttk.Button(footer_frame, text="Dark Theme", style="Modern.TButton", command=self.toggle_theme)
    ...
    def set_download_folder(self):
        ...
        ttk.Button(dialog, text="រុករក", style="Modern.TButton", command=browse_folder).pack(pady=5)
        ...
        ttk.Button(dialog, text="រក្សាទុក", style="Modern.TButton", command=confirm).pack(pady=10)
    ...
    def update_gui_colors(self):
        ...
        for widget in footer_frame.winfo_children():
            if isinstance(widget, ttk.Button):
                widget.configure(style="Modern.TButton")
        dialog_frames = [w for w in self.root.winfo_children() if isinstance(w, tk.Toplevel)]

...
class MediaDownloaderGUI:
    def __init__(self, root):
        ...
        tk.Label(self.header_frame, text="Frog Downloader (YouTube & Instagram)", font=("Helvetica", 16, "bold"), fg='white', bg=self.colors['header']).pack()
        ...
        tk.Label(cookie_frame, text="ឯកសារខូគី Instagram (សម្រាប់ខ្លឹមសារឯកជន):", font=self.label_font, fg=self.colors['text'], bg=self.colors['card']).grid(row=0, column=0, columnspan=2, sticky="w")
        ...
        tk.Label(cookie_frame, text="ចំណាំ: នាំចេញខូគីពីកម្មវិធីរុករករបស់អ្នកសម្រាប់ Instagram មាតិកាឯកជន។", font=self.secondary_font, fg=self.colors['text2'], bg=self.colors['card']).grid(row=3, column=0, columnspan=2, sticky="w")
        ...
        tk.Label(single_frame, text="ទាញយកវីឌីអូ ឬ ប្រកាសតែមួយ (YouTube/Instagram)", font=self.label_font, fg=self.colors['text'], bg=self.colors['card']).grid(row=0, column=0, columnspan=2, sticky="w")
        tk.Label(single_frame, text="បញ្ចូល URL (YouTube ឬ Instagram):", font=self.secondary_font, fg=self.colors['text2'], bg=self.colors['card']).grid(row=1, column=0, sticky="w", pady=5)
        ...
        tk.Label(multi_frame, text="ទាញយកវីឌីអូ ឬ ប្រកាសច្រើន (YouTube/Instagram)", font=self.label_font, fg=self.colors['text'], bg=self.colors['card']).grid(row=0, column=0, columnspan=2, sticky="w")
        ...
        tk.Label(channel_frame, text="ទាញយកឆានែល បញ្ជីចាក់ ឬ ប្រវត្តិរូប Instagram", font=self.label_font, fg=self.colors['text'], bg=self.colors['card']).grid(row=0, column=0, columnspan=2, sticky="w")
        tk.Label(channel_frame, text="បញ្ចូល URL ឆានែល/បញ្ជីចាក់/ប្រវត្តិរូប:", font=self.secondary_font, fg=self.colors['text2'], bg=self.colors['card']).grid(row=1, column=0, sticky="w", pady=5)
        ...
        tk.Button(channel_frame, text="ទាញយកឆានែល/បញ្ជីចាក់/ប្រវត្តិរូប", font=self.button_font, bg=self.colors['accent'], fg='white', activebackground=self.colors['accent_active'], relief="flat", command=self.download_channel).grid(row=2, column=1, padx=5)
        ...
        self.log_status("ចំណាំ: សម្រាប់ Instagram មាតិកាឯកជន សូមបញ្ចូលឯកសារខូគី។ នាំចេញខូគីពីកម្មវិធីរុករករបស់អ្នក។")
    ...
    def download_channel_thread(self, url):
        self.log_status(f"ចាប់ផ្តើមទាញយកឆានែល/បញ្ជីចាក់/ប្រវត្តិរូប: {url}")
        opts = self.get_ydl_opts()
        opts['noplaylist'] = False  # Allow playlists/channels/profiles
        with yt_dlp.YoutubeDL(opts) as ydl:
            try:
                ydl.download([url])
                self.log_status(f"✅ ទាញយកឆានែល/បញ្ជីចាក់/ប្រវត្តិរូបបានសម្រេច: {url} ➜ រក្សាទុកនៅ '{DOWNLOAD_FOLDER}'")
            except Exception as e:
                self.log_status(f"❌ បរាជ័យ: {url}: {e}")
            finally:
                self.root.after(0, self.progress.stop)
